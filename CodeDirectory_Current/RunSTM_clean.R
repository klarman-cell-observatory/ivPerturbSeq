...
out<-runSTM(seur,c(cell),numberPCs=0,forForm=c("perturbation","batch"),numTopics=numTop,genes=c(),minPert=10,getMeta=T)
...
##Tests for differences in each coefficient of the STM output
##
testSTM_all<-function(res,meta,toUse=c(),form="~batch+perturbation",reference="GFP",variable="perturbation",as.df=T,rawtheta=T,logistic=T,numRep=1000,topics=c(),getResid=F,form1=Topic~batch+perturbation+nGene,form2=Topic~batch+nGene,getFit=F,test="anova")
{
print("Get theta")
theta=res$theta
...
if(test=="normal")
{
fit=normal_test(Topic~perturbation|batch,data=meta,distribution="approximate")
}
...
if(test=="normal")
{
for(i in 1:length(results)){print(i);pval<-pvalue(results[[i]]);pval<-attr(pval, "conf.int")[2];pvals<-c(pvals,pval);names<-c(names,sub("^","Topic",i));print(pvals);print(names)}
}
